...
- `H` swaps the cursor for a hand. Hold the left button to pick up fruit, a hat, a ball, the drum, the music box or a duck, and release it to put the thing down. Release while the mouse is moving to throw it, except a fruit, which just drops where you let go. To turn the camera, drag with the left button, or with the right button while the hand is out.
